...
class OperationHelper(BasePage):
    ...
    def enter_pass(self, word):
        logging.info(f"Send {word} to element{TestSearchLocator.LOCATOR_PASS_FIELD[1]}")
    ...
    def get_error_text(self):
        error_field = self.find_element(TestSearchLocator.LOCATOR_ERROR_FIELD, time=5)
        text = error_field.text
        logging.info(f"We find {text}in error field {TestSearchLocator.LOCATOR_ERROR_FIELD}")
        return text
